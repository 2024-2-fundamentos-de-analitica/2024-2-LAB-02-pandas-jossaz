...
def pregunta_09():
    ...
    # Opcionalmente:
    # Convertir a tipo datetime especificando el formato
    #df['c3'] = pd.to_datetime(df['c3'], format='%Y-%m-%d')
    # Se extrae el año de la columna c3(como datatime)
    #df['year'] = df['c3'].dt.year

    # Se extraen solo los 4 primeros caracteres de c3(la fecha)
    df['year'] = df['c3'].str[:4] 
